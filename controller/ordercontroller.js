...
const createOrderData = async (
  ...
  const orderDateTimeWithTime = moment(orderDateTime)
    .set("hour", orderTime.split(":")[0])
    .set("minute", orderTime.split(":")[1]);
...
const checkout = async (req, res) => {
  try {
    ...
    console.error(
      "Error fetching user addresses and cart data for checkout:",
      error
    );
    res.status(500).json({ success: false, message: "Internal Server Error" });
  }
...
const createOrder = async (req, res) => {
  try {
    ...
    if (paymentMethod === "Cash on Delivery") {
      ...
      setTimeout(() => {
        return res.render("./orders/confirm", {
          pageTitle: "success",
          user: req.session.user,
          paymentMethod,
        });
...
const userOrder = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.render("./orders/userorder", {
        pageTitle: "userorder",
        user: null,
        orders: [],
        totalPages: 0,
...
const cancelProduct = async (req, res) => {
  ...
  try {
    ...
    const productName = cancelledProduct.product
      ? cancelledProduct.product.productName
      : "Unknown Product";
    const price = isNaN(cancelledProduct.product?.price)
      ? 0
      : cancelledProduct.product.price;
    const tax = isNaN(cancelledProduct.product?.tax)
      ? 0
      : cancelledProduct.product.tax;
    const cancelledAmount = cancelledProduct.quantity * price + tax;
    const updatedProductQuantity =
      cancelledProduct.product.quantity + cancelledProduct.quantity;
...
const downloadInvoice = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=invoice_${formattedDate}.pdf`
    );
